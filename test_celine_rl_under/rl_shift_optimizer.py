"""
Reinforcement Learning Agent for Under-Demand Employee Shift Scheduling
Scenario: More capacity than demand - learn to fairly distribute limited work

Key RL Advantages in Under-Demand:
1. Fair work distribution (learn who gets limited shifts)
2. Balanced utilization (avoid some employees getting all work while others get none)
3. Optimal assignment patterns (learn best combinations)
4. Adaptive fairness strategies (multi-objective learning)
"""
...
class UnderDemandRLAgent:
    def __init__(self, forecast_file, employees_file, learning_rate=0.1, epsilon=0.3):
        """
        Initialize RL Agent for under-demand shift scheduling
        """
        ...
        # Employee capacity (not requirements in under-demand)
        ...
        print(f"Under-demand scenario: {total_demand} demand vs {total_capacity} capacity")
        ...
        print("🤖 RL Agent initialized for under-demand fairness optimization!")
    
    def _initialize_q_table(self):
        """Initialize Q-table for under-demand scenarios"""
    ...
    def get_state_key(self, day, shift, employee, current_schedule):
        """Create state representation for under-demand fairness"""
        ...
        balance_level = "balanced" if util_std <= 0.2 else "unbalanced"
    ...
    def calculate_reward(self, action, day, shift, employee, current_schedule):
        """
        Under-demand reward function focusing on fair work distribution
        """
        reward = 0
        
        if action == 'assign':
            # 1. Demand coverage reward (must be met)
            ...
            if currently_assigned < required:
                reward += 20  # Essential to meet demand
            else:
                reward -= 100  # Cannot overstaff in under-demand
            ...
            if emp_total < capacity:
                # Good to give work up to capacity
                utilization = (emp_total + 1) / capacity
                reward += utilization * 10  # Reward higher utilization
            else:
                reward -= 50  # Cannot exceed capacity
            
            # 3. Fairness in work distribution (KEY METRIC)
            all_utilizations = []
            for emp in self.employees:
                emp_assigned = sum(
                    sum(current_schedule[emp][d][s] for s in self.shifts) 
            ...
            if all_utilizations:
                ...
                if util_std <= 0.1:
                    reward += 30  # Excellent balance
                elif util_std <= 0.2:
                    reward += 15  # Good balance
                else:
                    reward -= util_std * 20  # Penalty for imbalance
                
                # Bonus for avoiding extreme disparities
                min_util = min(all_utilizations)
                max_util = max(all_utilizations)
                if max_util - min_util <= 0.2:
                    reward += 25  # Great fairness
            
            # 4. Consecutive shift consideration
            daily_shifts = sum(current_schedule[employee][day][s] for s in self.shifts)
            if daily_shifts == 1:  # Adding second shift
                reward += 5  # Good for employee efficiency
            elif daily_shifts >= 2:
                reward -= 10  # Avoid overloading single employee per day
            ...
            if currently_assigned >= required:
                reward += 5  # Good to not overstaff
            else:
                reward -= 15  # Bad to leave demand unmet
        
        return reward
    
    def choose_action(self, state_key):
        """Epsilon-greedy action selection with fairness bias"""
    ...
    def create_initial_schedule(self):
        ...
        for emp in self.employees:
            ...
            for day in self.days:
                schedule[day] = {}
                for shift in self.shifts:
                    schedule[day][shift] = 0
        return schedule
    
    def is_valid_assignment(self, employee, day, shift, current_schedule):
        """Check if assignment is valid under under-demand constraints"""
        ...
        # Check daily shift limits (max 2 shifts per day)
        daily_shifts = sum(current_schedule[employee][day][s] for s in self.shifts)
        if daily_shifts >= 2:
            return False
        
        # Check consecutive shift constraint
        shifts_dict = {s: current_schedule[employee][day][s] for s in self.shifts}
        if shift == 'shift_evening' and shifts_dict['shift_morning'] == 1 and shifts_dict['shift_afternoon'] == 0:
            return False  # Cannot work morning and evening without afternoon
        
        # Check demand fulfillment (don't overfill)
        required = self.demand[(day, shift)]
        currently_assigned = sum(
            current_schedule[emp][day][shift] for emp in self.employees
        )
    ...
    def evaluate_schedule(self, schedule):
        """Evaluate schedule quality focusing on under-demand fairness"""
        score = 0
        ...
        if demand_violations == 0:
            score += 1000  # Perfect demand coverage
        else:
            score -= demand_violations * 100  # Heavy penalty
        
        # 2. Fairness in work distribution
        utilizations = []
        for emp in self.employees:
            total_assigned = sum(
        ...
        if utilizations:
            ...
            score += (1.0 - util_std) * 500  # Higher score for lower std deviation
            
            # Bonus for high overall utilization
            avg_utilization = np.mean(utilizations)
            score += avg_utilization * 200
            
            # Penalty for extreme disparities
            min_util = min(utilizations)
            max_util = max(utilizations)
            if max_util - min_util > 0.3:
                score -= (max_util - min_util) * 300
        ...
        for emp in self.employees:
            ...
            # Check daily shift constraints
            for day in self.days:
                daily_shifts = sum(schedule[emp][day][s] for s in self.shifts)
                if daily_shifts > 2:
                    constraint_violations += daily_shifts - 2
        
        score -= constraint_violations * 50
    ...
    def print_solution_summary(self):
        ...
        print("\n=== RL UNDER-DEMAND SCHEDULING SOLUTION ===")
        ...
        if all_covered:
            print(f"\n✅ Perfect demand coverage achieved!")
        else:
            print(f"\n❌ Demand coverage issues detected")
...
def main():
    ...
    # Create RL agent
    agent = UnderDemandRLAgent(forecast_file, employees_file)
    
    print("Starting RL training for under-demand scenario...")
    
    # Train the agent
    try:
        best_schedule = agent.train(episodes=1000)
